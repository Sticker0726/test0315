...
class DbMgr():
    ...
    def get_data(self):
        ''' 查询用户表，文章表
        '''
        self._users = self._ses.query('"user"', self._Member.mid, self._Member.email, self._Member.nickname).all()
        self._articles = self._ses.query(self._Article).all()
        self._articles = [('ar', i.arid, i.title, i.tags, i.description) for i in self._articles]
